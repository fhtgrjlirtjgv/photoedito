...
class Widget(QMainWindow):
    def   __init__(self):
        ...
        self.ui.pushButton_6.clicked.connect(self.do_black_white)
        self.ui.image_list.currentRowChanged.connect(self.show_chosen_image)
        self.ui.mirror_button.clicked.connect(self.do_mirror)
        self.ui.sharpness_button.clicked.connect(self.do_blur)
        self.ui.right_button.clicked.connect(self.do_right)
        self.ui.left_button.clicked.connect(self.do_left)
    ...
    def do_mirror(self):
        self.new_image = self.image.transpose(Image.FLIP_LEFT_RIGHT)
        self.save_image()
        self.show_image()


    def do_blur(self):
        self.new_image = self.image.filter(ImageFilter.BLUR)
        self.save_image()
        self.show_image()


    def do_right(self):
        self.new_image = self.image.transpose(Image.ROTATE_90)
        self.save_image()
        self.show_image()
    ...
    def do_left(self):
        self.new_image = self.image.transpose(Image.ROTATE_270)
        self.save_image()
        self.show_image()
